...
export const battles = {
  ...
  actions: {
    getCardAndDeckPlayers(
      { rootState, rootGetters, state, getters },
      { board, card }
    ) {
      let cards =
        board === "opponent" ? state.opponentsCard : rootState.decks.decksList;
      let deckIndex =
        board === "opponent"
          ? ""
          : cards?.findIndex(
              (deck) => deck.name === rootGetters["decks/getFavoriteDeck"].name
            );

      let cardIndex =
        board === "opponent"
          ? cards.cards.findIndex((opponent) => opponent.name === card.name)
          : cards
              .at(deckIndex)
              .cards.findIndex((user) => user.name === card.name);

      return {
        cards: cards,
        deckIndex: deckIndex,
        cardIndex: cardIndex,
        allCards: rootState.cards.cardList,
      };
    },
    damageAttack({ commit, state, dispatch }, { board, card }) {
      commit("updateCardAttack", { key: board, card: card });
      const promise = dispatch("getCardAndDeckPlayers", {
        board: board,
        card: card,
      });
      ...
      promise.then((response) => {
        ...
        let boardNext = "user";
        if (board === "opponent") {
          cards.cards.at(cardIndex).stats.health =
            cards.cards.at(cardIndex).stats.health -
            state.cardAttack["user"].stats.damage;
        ...
        if (board === "user") {
          cards.at(deckIndex).cards.at(cardIndex).stats.health =
            cards.at(deckIndex).cards.at(cardIndex).stats.health -
            state.cardAttack["opponent"].stats.damage;
    ...
    healingCard({ commit, state, dispatch }, { board, card }) {
      ...
      promise.then((response) => {
        ...
        let sameCard = allCards.find(
          (cardPlayer) => cardPlayer.name === card.name
        );
        let fullhp = allCards.find(
          (cardPlayer) => cardPlayer.name === card.name
        );
        let root = false;
        ...
        if (sameCard.name === state.cardAttack[board].name) {
          commit(
            "notify/updateNotify",
            { visible: true, message: "Нельзя лечить самого себя" },
            { root: true }
          );
          return;
        }
        if (fullhp.stats.health === card.stats.health) {
          commit(
            "notify/updateNotify",
            { visible: true, message: "У этой карты полное здоровье" },
            { root: true }
          );
          return;
        }
        ...
        if (board === "user") {
          cards.at(deckIndex).cards.at(cardIndex).stats.health =
            cards.at(deckIndex).cards.at(cardIndex).stats.health +
            state.cardAttack[board].stats.healing;
          if (
            cards.at(deckIndex).cards.at(cardIndex).stats.health >=
            fullhp.stats.health
          ) {
    ...
    takeCardEventCard(
      ...
      if (event === "damage") {
        commit(
          ...
          {
            visible: true,
            message: "Кликните на карту оппонента, которую хотите атаковать",
          },
          { root: true }
        );
        if (board === "user") {
      ...
      if (event === "healing") {
        commit(
          "notify/updateNotify",
          {
            visible: true,
            message:
              "Кликните на карту из вашей колоды, которую хотите излечить",
          },
          { root: true }
        );
        return;
